    ...
    while True:
        ...
        if op > 2 or op < 0:
            ...
            if op == 1:
                ...
                break
            ErickCypher(word=input('Palavra : '), key=int(input('Chave : ')), Enc_Dec=CypherMode)
            os.system("pause")
...
def CreateMask(key, key_Alpha, lenWord, Enc_Dec):
    """
    :Param:
        ...
        Enc_Dec:    bol - False:(desencriptar) inverte os bits do binario da key
...
def ErickCypher(key, word, Enc_Dec=True):
    ...
    if key_Alpha == 0 or key_Alpha == len(Alpha):
        ...
        mask = CreateMask(key, key_Alpha, len(word), Enc_Dec)
        keyLetters = SetKeyLetters(word)
        encriptedWord = ''
